...
class We7_couponModuleSite extends WeModuleSite {
			...
			if (pdo_insert('mc_card_members', $record)) {
				...
				if (!empty($card['grant']['coupon']) && is_array($card['grant']['coupon'])) {
					foreach ($card['grant']['coupon'] as $grant_coupon) {
						we7_coupon_activity_coupon_grant($grant_coupon['coupon'], $uid);
					}
				}
				exit('success');
			}
	...
	public function doWebCouponmanage() {
		...
		if ($op == 'display') {
			...
			foreach ($couponlist as &$row) {
				...
				$row['type'] = we7_coupon_activity_coupon_type_label($row['type']);
				if ($row['status'] == 1) {
		...
		if ($op == 'post') {
			...
			$coupon_title = we7_coupon_activity_coupon_type_label($type);
		...
		if ($op == 'publish') {
			...
			if (empty($coupon['is_display'])) {
				message(error(-1, '卡券已下架'), '', 'ajax');
			}
		...
		if ($op == 'detail') {
			...
			$coupon_title = $coupon_title[0]; //卡券类型标题
			$groups = mc_fans_groups();
			$colors = we7_coupon_activity_coupon_colors();
		...
		if ($op == 'sync') {
			...
			we7_coupon_activity_coupon_sync();
	...
	public function doWebCouponconsume() {
		...
		if ($op == 'display') {
			...
			if (empty($coupon['is_display'])) {
				message(error(-1, '卡券已下架,核销失败'), '', 'ajax');
			}
	...
	public function doWebCouponmarket() {
		...
		$coupon_api = new coupon();
		$type_names = we7_coupon_activity_get_coupon_label();
		if ($op == 'checkcoupon') {
			$coupon_id = intval($_GPC['coupon']);
			$coupon = we7_coupon_activity_coupon_info($coupon_id);
			...
			if (checksubmit('submit')) {
				...
				if (empty($id)) {
					...
					foreach ($openids as $openid) {
						...
						$coupon_info = we7_coupon_activity_coupon_info($post['coupons']);
	...
	public function doWebCouponexchange() {
		...
		$op = !empty($_GPC['op']) ? $_GPC['op'] : 'display';
		if ($op == 'coupon_info') {
			$coupon = we7_coupon_activity_coupon_info(intval($_GPC['id']));
		...
		if ($op == 'post') {
			...
			$coupons = pdo_fetchall("SELECT * FROM ". tablename('coupon')." WHERE uniacid = :uniacid AND is_display = 1 AND source = ".COUPON_TYPE, array(':uniacid' => $_W['uniacid']), 'id');
		...
		if ($op == 'display') {
			...
			if (!empty($list)) {
				foreach($list as &$ex) {
					$ex['coupon'] = we7_coupon_activity_coupon_info($ex['extra']);
	...
	public function doWebPaycenter() {
		...
		if ($op == 'pay') {
			...
			$card_set = we7_coupon_card_setting();
	...
	public function doMobileCard() {
		...
		checkauth();
		//未读取的消息条数
		$notice_count = we7_coupon_card_notice_stat();
		$op = trim($_GPC['op']) ? trim($_GPC['op']) : 'mycard';
		$setting = pdo_get('mc_card', array('uniacid' => $_W['uniacid']));
		if ($op == 'sign_display') {
			$title = '签到-会员卡';
			$credit_set = we7_coupon_card_credit_setting();
		...
		if ($op == 'sign_strategy') {
			$set = we7_coupon_card_credit_setting();
			$content = $set['content'];
		}
		...
		if ($op == 'notice') {
			...
			if ($_W['isajax']) {
				...
				if ($id > 0) {
					...
					$total = we7_coupon_card_notice_stat();
					exit($total);
				}
			}
			$psize = 20;
		...
		if ($op == 'mycard') {
			...
			$total = count($coupons);
			$activity_setting = we7_coupon_card_params_setting('cardActivity');
			$notice_count = we7_coupon_card_notice_stat();
		...
		if ($op == 'consume') {
			$card_settings = we7_coupon_card_setting();
			...
			if (checksubmit()) {
				...
				$final_fee = we7_coupon_card_discount_fee($fee);
				$data = array(
					'uniacid' => $_W['uniacid'],
					'clerk_id' => 0,
					'clerk_type' => 3,
	...
	public function doMobileActivity() {
		...
		$type_names = we7_coupon_activity_coupon_type_label();
		we7_coupon_activity_coupon_type_init();
		$colors = we7_coupon_activity_coupon_colors();
		...
		if ($activity_type == 'coupon') {
			...
			if($op == 'display') {
				...
				foreach ($exchanges as $key => &$list) {
					$coupon_info = we7_coupon_activity_coupon_info($list['extra']);
			...
			if ($op == 'exchange') {
				...
				if (is_error($status)) {
					...
					if ($activity_exchange['credittype'] == 'credit1') {
						...
						$recharges_set = we7_coupon_card_params_setting('cardRecharge');
						if (empty($recharges_set['params']['recharge_type'])) {
			...
			if ($op == 'use') {
				...
				$coupon_info = we7_coupon_activity_coupon_info(trim($coupon_record['couponid']));
                if (empty($coupon_info['is_display'])) {
                    message('该卡券已下架');
                }
			...
			if ($op == 'detail') {
				...
				$coupon_info = we7_coupon_activity_coupon_info($couponid);
			...
			if ($op == 'post') {
				...
				$goods = we7_coupon_activity_exchange_info($id, $_W['uniacid']);
				if (empty($goods)){
				...
				$ret = we7_coupon_activity_goods_grant($_W['member']['uid'], $id);
				if (is_error($ret)) {
					message($ret, '', 'ajax');
				}
	...
	public function doMobileClerkcardconsume() {
		...
		if($_W['isajax']) {
			...
			$status = we7_coupon_activity_coupon_use($record['couponid'], $record['id'], 'paycenter');
			if (!is_error($status)) {
				message(error('0', ''), '', 'ajax');
			} else {
				message(error('-1', $status['message']),'' , 'ajax');
	...
	public function doMobileConsume() {
		...
		if (checksubmit()) {
			...
			$record = pdo_get('coupon_record', array('code' => $code));
			$status = we7_coupon_activity_coupon_use($card['id'], $record['id'], 'paycenter');
			if (is_error($status)) {
