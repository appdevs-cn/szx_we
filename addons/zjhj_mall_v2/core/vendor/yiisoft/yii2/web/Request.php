...
class Request extends \yii\base\Request
{
    ...
    {
        $res = isset($_SERVER['HTTPS']) && (strcasecmp($_SERVER['HTTPS'], 'on') === 0 || $_SERVER['HTTPS'] == 1)
            || isset($_SERVER['HTTP_X_FORWARDED_PROTO']) && strcasecmp($_SERVER['HTTP_X_FORWARDED_PROTO'], 'https') === 0;

        //wdcp环境无法判断https补丁
        $host_name = Yii::$app->request->hostName;
        $wdcp_patch_file = Yii::$app->basePath . '/patch/wdcp.json';
        if (file_exists($wdcp_patch_file) && $host_name) {
            $wdcp_patch = file_get_contents($wdcp_patch_file);
            if ($wdcp_patch) {
                $wdcp_patch = json_decode($wdcp_patch, true);
                if ($wdcp_patch && is_array($wdcp_patch) && in_array($host_name, $wdcp_patch)) {
                    return true;
                }
            }
        }
