...
class TeamForm extends Model
{
    ...
    public static function team($store_id,$user_id)
    {
        ...
        $list = User::find()->alias('u')
            ->where(['and', ['u.is_delete' => 0], ['>', 'u.parent_id', 0]])
...
//        VarDumper::dump($this->user_id,3,1);
        ...
        foreach($list as $index=>$value){
            ...
            if(in_array($value['user_id'],$team_arr['f_c'])){
            ...
            if(in_array($value['user_id'],$team_arr['s_c'])){
            ...
            if(in_array($value['user_id'],$team_arr['t_c'])){
