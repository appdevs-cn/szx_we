...
class OrderPayDataForm extends Model
{
    ...
    private function setReturnData($order)
    {
        ...
        $order_detail_list = OrderDetail::find()->alias('od')->leftJoin(['g' => Goods::tableName()], 'od.goods_id=g.id')
            ...
            ->asArray()
            ->select('g.individual_share,g.share_commission_first,g.share_commission_second,g.share_commission_third,od.total_price')
            ->all();
        ...
        foreach ($order_detail_list as $item) {
            $rate_first = $setting->first;
            $rate_second = $setting->second;
            $rate_third = $setting->third;
            ...
            $share_commission_money_first += doubleval($item['total_price']) * $rate_first / 100;
            $share_commission_money_second += doubleval($item['total_price']) * $rate_second / 100;
            $share_commission_money_third += doubleval($item['total_price']) * $rate_third / 100;
        }
