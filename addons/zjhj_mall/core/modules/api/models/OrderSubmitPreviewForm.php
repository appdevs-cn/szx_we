...
//                    $new_item->give = ($new_item->price * ($give/100)) * $store->integral;
                ...
                if (strpos($forehead, '%') !== false) {
                    ...
                    if ($integral->more == '1') {
                        $resIntegral['forehead'] += sprintf("%.2f", ($forehead / 100) * $new_item->price);
                    ...
                    if ($integral->more == '1') {
                        $resIntegral['forehead'] += sprintf("%.2f", ($forehead * $new_item->num));
                    } elseif ($integral->more != '1' && !in_array($goods->id, $goodsIds)) {
...
//                $goods_item->give = ($goods_item->price * ($give/100)) * $store->integral;
            ...
            if (strpos($forehead, '%') !== false) {
                $forehead = trim($forehead, '%');
                if ($integral->more == '1') {
                    $resIntegral['forehead'] = sprintf("%.2f", ($forehead / 100) * $goods_item->price);
                } else {
                    $resIntegral['forehead'] = sprintf("%.2f", ($forehead / 100) * (empty($goods_attr_info['price']) ? $goods->price : $goods_attr_info['price']));
                }
            } else {
