...
class GoodsListForm extends Model
{
    ...
    public function search()
    {
        ...
        $od_query = OrderDetail::find()->alias('od')
            ...
            ->where(['od.is_delete' => 0, 'o.store_id' => $this->store_id, 'o.is_pay' => 1])->groupBy('od.goods_id')->select('SUM(od.num) num,od.goods_id');

        $list = $query->leftJoin(['gp' => $pic_query], 'gp.goods_id=g.id')
            ->leftJoin(['gn' => $od_query], 'gn.goods_id=g.id')
            ->select('g.id,g.name,g.price,g.cover_pic pic_url')
