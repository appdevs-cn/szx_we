                    ...
                    <?php if ($value['status'] == 1): ?>
                        ...
                        <?php else: ?>
                            <?php if ($setting->level >= 1): ?>
                                <div><a class="team" data-index="<?=$value['id']?>" data-level="1" href="javascript:" data-toggle="modal" data-target="#exampleModal"><?=$setting->first_name?$setting->first_name:"一级"?>：<?= $value['first'] ?></a></div>
                            <?php endif; ?>
                            <?php if ($setting->level >= 2): ?>
                                <div><a class="team" data-index="<?=$value['id']?>" data-level="2" href="javascript:" data-toggle="modal" data-target="#exampleModal"><?=$setting->second_name?$setting->second_name:"二级"?>：<?= $value['second'] ?></a></div>
                            <?php endif; ?>
                            <?php if ($setting->level == 3): ?>
                                <div><a class="team" data-index="<?=$value['id']?>" data-level="3" href="javascript:" data-toggle="modal" data-target="#exampleModal"><?=$setting->third_name?$setting->third_name:"三级"?>：<?= $value['third'] ?></a></div>
                            <?php endif; ?>
                        <?php endif; ?>
                    <?php endif; ?>
                </td>
                    ...
                    <div class="btn btn-group" role="group">
                        ...
                        <div class="dropdown-menu dropdown-menu-left" aria-labelledby="dropdownMenuButton">
                            ...
                            <?php elseif($value['status'] == 1): ?>
                                <a class="dropdown-item" href="<?=$urlManager->createUrl(['mch/order/index','keyword'=>$value['nickname']])?>">会员订单</a>
                                <a class="dropdown-item" href="<?=$urlManager->createUrl(['mch/share/cash','id'=>$value['id']])?>">提现明细</a>
                                <a class="dropdown-item del" href="javascript:" data-url="<?=$urlManager->createUrl(['mch/share/del','id'=>$value['id']])?>" data-content="是否删除分销商？">删除分销商</a>
                            <?php endif; ?>
                        </div>
                    </div>
                </td>
