...
<div class="main-body p-3">
    <a href="<?= $urlManager->createUrl(['mch/store/express-edit']) ?>" class="btn btn-primary mb-3">
        <i class="iconfont icon-playlistadd"></i>添加面单打印设置</a>
    <a href="javascript:" class="btn btn-primary mb-3" data-toggle="modal" data-target="#sender">
        <i class="iconfont icon-playlistadd"></i>发件人信息</a>
    <a href="javascript:" class="btn btn-primary mb-3" onclick="CheckIsInstall()">检测是否安装打印插件</a>
        ...
        <?php foreach ($list as $index => $value): ?>
            <tr>
                ...
                <td>
                    ...
                    <div>地址：<?=$value['sender_province'].$value['sender_city'].$value['sender_area'].$value['sender_address']?></div>
                </td>
                <td>
    ...
    <!-- Modal -->
    <div class="modal fade" id="install" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                ...
                <div class="modal-body">
                    <font color='#FF00FF'>CLodop云打印服务(localhost本地)未安装启动!点击这里<a href='http://www.mtsoftware.cn/download.html' target='_blank'>执行安装</a>,安装后请刷新页面。</font>
                </div>
            </div>
        </div>
    </div>
