...
class CloudAdmin
{
	...
	{
		$admin = Admin::findOne(['id' => \Yii::$app->request->get('id'), 'is_delete' => 0]);
		if (!$admin) {
			$admin = new Admin();
		}
		...
		return ['model' => $admin, 'account_count' => $account_count, 'account_max' => $account_max, 'account_over_max' => $account_over_max, 'permission_list' => AdminPermission::getList()];
	}
