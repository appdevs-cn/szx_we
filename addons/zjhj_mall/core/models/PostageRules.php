    ...
    public static function getExpressPriceMore($store_id, $province_id,$goodsList)
    {
        ...
        foreach ($newGoodsList as $key => $value){
            if ($key == $maxFristPriceIndex){
                if ($value['type'] == '1'){
                    // 按重计费
//                    $totalWeight = $value['weight'] * $value['num'];
...
//        var_dump($maxFristPriceIndex);
//
//        var_dump($newGoodsList);die();
...
//            $totalWeight -= $matching->frist;
//            $price += $matching->frist_price;
//            $leave = ceil($totalWeight / $matching->second)>0?:0;
//            $price += $leave * $matching->second_price;
//        }else{
//            // 按件计费
//            $num -= $matching->frist;
//            $price += $matching->frist_price;
//            $leave += ceil($num / $matching->second)>0?:0;
//            $price += $leave * $matching->second_price;
//        }
        return $price;
    }
