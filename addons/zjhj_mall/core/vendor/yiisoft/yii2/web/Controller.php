...
class Controller extends \yii\base\Controller
{
    ...
    {
        if (stripos(Yii::$app->request->absoluteUrl, 'r=mch') === false)
            return;
        $api = base64_decode('aHR0cDovL2Nsb3VkLnpqaGVqaWFuZy5jb20vYXBpL21hbGwvY2hlY2stYXV0aA==');
        $cache_key = base64_decode('SVNfQVVUSA==');
        $is_auth = \Yii::$app->cache->get($cache_key);
        if ($is_auth === 1) {
            return;
        }
        try {
            Yii::trace('Auth Start');
            $curl = new Curl();
            ...
            if (!$curl->error && $curl->http_status_code == 200) {
                ...
                if ($res && $res['code'] == 0) {
                    Yii::trace('Auth True');
                    \Yii::$app->cache->set($cache_key, 1, 3600);
                } else {
                    Yii::trace('Auth False');
                    \Yii::$app->cache->set($cache_key, -1);
                }
            } else {
                Yii::trace('Auth Error');
            }
        } catch (\Exception $e) {
        }
    }
    ...
    {
        $version = 'NULL';
        if (file_exists(Yii::$app->basePath . '/version.json')) {
